      ...
      <aside className="w-64 h-full bg-card border-r border-border flex flex-col pt-6 overflow-hidden">
        <div className="px-6 mb-8 flex items-center space-x-3">
          ...
          <div className="flex flex-col">
            <span className="text-xl font-bold text-primary tracking-tight leading-none">SNS</span>
              ...
              <div className="space-y-1">
                {group.items.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`flex items-center space-x-3 px-3 py-2 text-sm font-medium rounded-sm transition-colors ${pathname === item.href
